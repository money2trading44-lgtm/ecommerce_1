...
def send_new_order_notification(order):
    ...
    try:
        ...
        if not all([settings.EMAIL_HOST_USER, settings.EMAIL_HOST_PASSWORD]):
            print("❌ Configuration email manquante - notification ignorée")
            return
...
"""
        ...
        # 🔥 AJOUTE UN TIMEOUT pour éviter le blocage
        send_mail(
            subject,
            message.strip(),
            settings.DEFAULT_FROM_EMAIL,
            [settings.ADMIN_EMAIL],
            fail_silently=True,  # ⚠️ CHANGE À True pour ne pas crasher
        )
